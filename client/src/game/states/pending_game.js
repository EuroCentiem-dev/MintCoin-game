...
var characterSquareStartingX = 570;
var characterSquareStartingY = 150;
...
PendingGame.prototype = {
	...
	create: function() {
		...
		var nameLabel5=this.add.text(50,25,'Pending Game loaded....',{font: '15px Arial', fill:'#ffffff'});
		...
		// set specific pending game items

		var backdrop = game.add.image(game.world.centerX,game.world.centerY, TEXTURES, "lobby/backdrop.png");
		backdrop.anchor.setTo(0.5,0.5);

		this.startGameButton = game.add.button(game.world.centerX + 135, game.world.centerY + 150, TEXTURES, null, this,
			"lobby/buttons/start_game_button_03.png", "lobby/buttons/start_game_button_03.png");
		this.startGameButton.anchor.setTo(0.5,0.5);
		this.leaveGameButton = game.add.button(game.world.centerX + 135, game.world.centerY + 200 , TEXTURES, this.leaveGameAction, null,
			"lobby/buttons/leave_game_button_02.png", "lobby/buttons/leave_game_button_01.png");
		this.leaveGameButton.anchor.setTo(0.5,0.5);
	...
	populateCharacterSquares: function(data) {
		...
		for(var playerId in data.players) {
			...
			this.characterImages[playerId] = game.add.image(this.characterSquares[this.numPlayersInGame].position.x + characterOffsetX,
				this.characterSquares[this.numPlayersInGame].position.y + characterOffsetY, "mint_textures", "mint/user_m_" + color + ".png");
	...
	playerJoined: function(data) {
		...
		this.characterImages[data.id] = game.add.image(this.characterSquares[index].position.x + characterOffsetX,
		 this.characterSquares[index].position.y + characterOffsetY, "mint_textures", "mint/user_m_" +  data.color + ".png");
	...
	startGameAction: function() {
		console.log('client/src/game/states/pending_game.js - startGameAction	');
	...
	startGame: function(data) {
		socket.removeAllListeners();
		console.log('client/src/game/states/pending_game.js - startGame	' + data.mapName);
		console.log(this.id);
		game.state.start('boardOne', true, false, data.mapName, data.players, this.id, data.ActivePlayer);
	}
}
